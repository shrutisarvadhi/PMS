...
const { Employee, User } = database_1.models;
...
class EmployeeService {
    ...
    static async create(payload) {
        ...
        if (!existingUser) {
            throw new app_error_1.AppError(404, 'User not found for employee');
        }
        const existingEmployee = await Employee.findOne({ where: { userId } });
        if (existingEmployee) {
            throw new app_error_1.AppError(409, 'Employee already exists for this user');
        }
        if (managerId) {
            const manager = await Employee.findByPk(managerId);
            if (!manager) {
                throw new app_error_1.AppError(404, 'Manager not found');
            }
        }
